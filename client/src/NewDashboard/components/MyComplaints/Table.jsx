...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`inline-flex items-center text-xs font-semibold px-2.5 py-1 rounded-full border ${cls}`}
    >
      {s || "—"}
    </span>
  );
...
const Table = ({ items }) => {
  ...
  if (!items?.length) {
    return (
      <div className="rounded-xl border border-emerald-100 bg-white/80 p-6 text-center">
